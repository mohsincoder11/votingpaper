...
<div id="snackbarsuccess">   
	<div class="row">
		...
		<div class="col-md-12">
			<strong>Success!</strong> IBC Voting Added Successfully.
		</div>  		
...
<div id="snackbarentityid">          <strong>Success!</strong> IBC Voting Created Successfully.
	...
	<div class="container">
		<div class="slim-pageheader">
			<ol class="breadcrumb slim-breadcrumb">
				<li class="breadcrumb-item"><a href="{{route('addibc')}}">IBC Voting</a></li>
				<li class="breadcrumb-item">Add IBC Voting</li>
			</ol>
			<h6 class="slim-pagetitle">Add IBC Voting</h6>
		</div><!-- slim-pageheader -->
			...
			<div id="wizard2">
					...
					<div class="row">
						...
						<div class="col-lg-4">
							<div class="form-group mg-b-10-force">
								...
								<input id="meetingtitle" class="form-control" name="meetingtitle"  type="text" required="" placeholder="Enter Meeting Title">
				...
				<section>
					...
					<form method="POST" enctype="multipart/form-data" id="resolutionform" action="javascript:void(0)">
						<input type="hidden" name="createpollid2" id="createpollid2"> 
				...
				<section>
					...
					<div class="row pt-2">
						...
						<input type="hidden" id="resulttype" value="yn">
						<div class="col-lg-3 ">
							<div class="row">
								<div class="col-lg-1 pt-1">
									<input name="resultype" checked="" class="rd" id="yn" type="radio" value="yn">
								</div>
              ...
              if(currentIndex === 0) {
              		...
              		)
              	{
...

              		{
              			$.ajax({
              				type: "get",
              				url: "{{Route('addcreatepoll')}}",
              				data: {_token: CSRF_TOKEN,votingtype:$('#votingtype').val(),entity:$('#entity').val(),votingtitle:$('#votingtitle').val(),votestartdate:$('#votestartdate').val(),voteenddate:$('#voteenddate').val(),votestarttime:$('#votestarttime').val(),voteendtime:$('#voteendtime').val(),meetingdate:$('#meetingdate').val(),meetingtitle:$('#meetingtitle').val()}, 
              				dataType:'json',
              				success:function(data) {
              	 				//alert(data);
              	 				$('#createpollid').val(data.id);
              	 				$('#createpollid2').val(data.id);
              	 				$('#createpollsubmit').val(1);
              	 				$('#ibcidlabel1').text('IBC Voting Id : '+data.ibcid);
              	 				$('#ibcidlabel2').text('IBC Voting Id : '+data.ibcid);
              	 				$('#ibcidlabel3').text('IBC Voting Id : '+data.ibcid);
              	 				$('#ibcidlabel4').text('IBC Voting Id : '+data.ibcid);
              ...
              if(currentIndex == 1) {
              		var voterlist = $('#voterlist_length').val();
                            ...
                            if(currentIndex == 2) {
                            	...
                            	if(resdescription.isValid()) {
                            		return true;
                            	} else { resdescription.validate(); }
                            }
                            if(currentIndex == 3) {
	...
	$.ajax({
		...
		success:function(data) {
			...
			$.each(data, function (a, b) {  
				...
				$("#resolutiontablerow").append(
					'<tr><td>'+b.resorder+'</td><td width="10%">R'+b.resorder+'</td><td  width="50%">'+b.resdescription+'</td><td  width="30%">'+b.resolutiondetail+'</td><td  width="10%"><button class="btn btn-danger btn-sm rounded-circle deleteresolutionrow" data-toggle="modal" data-effect="effect-sign" id="'+b.id+'" data-toggle="tooltip-danger" title="Delete Record" data-placement="top"><i class="fas fa-trash"></i></button>  </td></tr>'
					);
			});
			createtable2();
			$('#loaderpage').hide();
