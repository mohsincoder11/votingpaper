...
<div id="deletemodalcall" class="modal fade ">
					...
					$.ajax({
						...
						success:function(data) {
							...
							$.each(array_name, function (a, b) {   
								...
								var start_time_sec = new Date(start_time).getTime();
								var current_time_sec = new Date().getTime();	
								var send_class='disabled'
								var remaining_voter=b.total_voter-b.voted;
								if(remaining_voter>0)
								{
									send_class='';
																//	console.log(remaining_voter);

								}
								var startdate=b.startdate.split("-").reverse().join("-");
								var enddate=b.enddate.split("-").reverse().join("-");
								$("#survey_table_row").append(
									'<tr><td>'+b.id+'</td><td><b>'+b.surveyid+'</b></td><td>'+b.surveytitle+'</td><td>'
									+startdate+' '+b.starttime+'</td><td>'+enddate+' '+b.endtime+'</td><td><b>'+b.total_voter+'</b></td><td><b>'+remaining_voter+'</b></td><td><a href="#" class="btn btn-primary btn-sm rounded-circle sendnotification tooltip2  '+send_class+'" id=' 
									+ b.id + '  ><span class="tooltiptext2">Send Notification</span><i class="fas fa-paper-plane"></i></a> </td></tr>'
									);
 ...
 language: {
 	...
 	lengthMenu: '_MENU_ items/page',
        ...
        $("#survey_table_row").on('click','.sendnotification',function(){
        	var survey_id=$(this).attr('id');
        	$('#loaderpage').show();
        	$.ajax({
        		type: "get",
        		url: "{{Route('send_remaining_survey_voter_notification')}}",
