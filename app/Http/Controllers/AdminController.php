...
class AdminController extends Controller
{
	...
	{
		$userdata = User::
		join('usermanages','usermanages.user_id','users.id')
		->where('role','!=',1)
	...
	public function get_all_adminnotification()
	{
		...
		$data = DB::table('notifications')->select('notifications.*', DB::raw('count(mobile_no) as noofuser'))->groupBy('title')->orderBy('id', 'desc');
		if($user->role == 2){
			$entity_ids=Entity::where('user_id', $user->id)->pluck('id')->toArray();
			$data=$data->whereIn('entity_id',$entity_ids);
		}
		$data=$data->get();
	...
	public function get_user_byentity(Request $request)
	{
		return response()->json(
			DB::table('electionvotinguserdatas')
				->select('memname', 'mobno')->where('entityid', $request->id)
				->get()->groupBy('mobno')
		);
	}
}
